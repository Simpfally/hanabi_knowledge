(* https://bitbucket.org/Abdallah/coredpro/src/master/analysis/src/Utils/Print.ml *)
open Printf
...
let list' ld id rd pr l = sprintf "%s%s%s" ld (String.concat id (PList.map pr l)) rd
let list pr = list' "[" "; " "]" pr
let unlines pr = list' "" "\n" "" pr
